module Spree
    ...
    private 
      ...
      def find_pdf_order (number)
        pdf_file_paths = []
        Find.find("tmp/order_prints/#{pdf_folder}") do |path| # Busco en order prints los pdf que correspondan al numero, en un caso normal solo habria 1
      ...
      def pdf_folder
        if (@order.tipo_factura == "consumidor_final" )
          'invoice_cfs'
        elsif (@order.tipo_factura == "factura_a")
          'invoice_as'
        elsif (@order.tipo_factura == "factura_b")
          'invoice_bs'
        else
          'invoice_mostradors'
        end
