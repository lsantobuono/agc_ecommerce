...
describe Spree::OrdersController, type: :controller do
  ...
  context "Order model mock" do
    ...
    context "#populate_combos" do
      let(:combo) { create :combo }
      it "should handle population" do
        expect do
          spree_post :populate_combos, combo_id: combo.id, "quantity_#{variant.id}": 5
        end.to change { user.orders.count }.by(1)
        order = user.orders.last
        # expect(response).to redirect_to spree.cart_path
        # expect(order.line_items.size).to eq(1)
        expect(ComboAplicado.count).to eq(1)
        line_item = order.line_items.first
        expect(line_item.variant_id).to eq(variant.id)
        expect(line_item.quantity).to eq(5)
        expect(line_item.combo_aplicado.combo).to eq(combo)
      end
    end
